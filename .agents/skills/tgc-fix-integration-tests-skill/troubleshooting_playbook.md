...
#### 11. Test Name Mismatch in Metadata
- **Symptom**: The test name in `tests_metadata_*.json` does not match the newly generated test name, or the generated test is not found.
- **Cause**: The resource was recently added or tests were refactored, causing the metadata to point to a test name that doesn't match the standard generated pattern (e.g., pointing to a test in another resource like `MessageBus` instead of `GoogleApiSource`).
- **Solution**: Explicitly list the expected test name in `tgc_tests` in the resource's YAML file. This forces the generator to include that specific test case in the generated test file.
- **Example**: In `GoogleApiSource.yaml`:
  ```yaml
  tgc_tests:
